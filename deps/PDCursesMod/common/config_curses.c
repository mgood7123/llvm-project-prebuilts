...
/* Code to 'configure' pdcurses_curses.h.  Run as,  e.g.,

./config_curses -DPDC_WIDE -DXCURSES

it will check to see if the existing pdcurses_curses.h has those options set
...
the existing 'pdcurses_curses.h',  snipping out the lines with config data,
and inserting new ones.  When we're done,  we remove 'pdcurses_curses.h' and
...
int main( const int argc, const char **argv)
{
   ...
   char buff[300];
   const char *curses_h_filename = "pdcurses_curses.h";
   ...
   if( verbose)
      printf( "pdcurses_curses.h updated\n");
   return( 0);
}
