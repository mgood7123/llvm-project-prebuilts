...
PDCurses distribution, please follow these guidelines:
 ...
 - Don't modify anything in the pdcurses directory or in other port
   directories. Don't modify pdcurses_curses.h or pdcurses_curspriv.h unless absolutely
   necessary. (And prefer modifying pdcurses_curspriv.h over pdcurses_curses.h.)
...
the VT100 (see pdcurses_curses.h). PDC_transform_line() must use this table; when
...
for all the ACS_ macros defined in pdcurses_curses.h; other values in the table
...
list of special keys found in pdcurses_curses.h (KEY_MIN through KEY_MAX). If
