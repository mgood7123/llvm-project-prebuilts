...
- 'pdcurses_curspriv.h' modified to allow C++ compilation.  52a2fdf518
...
- On gcc and mingw builds for WinCon, WinGUI,  and VT,  one can run 'make
  configure (options)' to revise 'pdcurses_curses.h' to use the specified options.
...
-  The 'mangling' of initscr() (see pdcurses_curses.h) that was done to ensure that
...
- The version number is now hardwired only in pdcurses_curses.h and configure.ac.
...
- Avoid conflict with ncurses by having apps define PDC_NCMOUSE before
  including pdcurses_curses.h to invoke the ncurses-style mouse interface,
...
- Eliminated pdcurses_term.h and terminfo.c, and moved mvcur() to move.c. These
...
- Dropped the XOPEN, SYSVcurses and BSDcurses defines from pdcurses_curses.h, as
...
- All of the A_* and WA_* attributes from X/Open are now defined in
  pdcurses_curses.h, although some are no-ops, pending the availablity of more
...
- Since 3.2, the panel library was simply a copy of the main library.
  This kludge is now dropped. (pdcurses_panel.h remains separate from pdcurses_curses.h.)
...
- Don't install pdcurses_curspriv.h, and don't include it with binary
  distributions.
...
- In the gcc makefiles, "$?" didn't really have the desired effect --
  ...
  pdcurses_curses.h, and pdcurses.a twice.  And apparently, this can mess up some
...
- The panel library has been folded into the main library. What this
  ...
  shared library. Note that pdcurses_panel.h remains separate from pdcurses_curses.h.
...
- Eliminated the platform defines (DOS, WIN32, OS2, XCURSES) from
  pdcurses_curses.h, except for X11-specific SCREEN elements and functions.
...
- pdcurses_panel.h functions sorted by name, as with other .h files; pdcurses_curses.h is
  no longer included by repeated inclusions of pdcurses_panel.h or pdcurses_term.h.
...
- Moved RIPPEDOFFLINE to pdcurses_curspriv.h.
...
- For backwards compatibility, a special module just for deprecated
  ...
  functions (in pdcurses_curspriv.h) are subject to change and should be avoided.
...
- Clean separation of platform-specific code from the rest. Outside of
  the platform directories, there remain only a few ifdefs in pdcurses_curses.h
  and pdcurses_curspriv.h.
...
- Removed all the EMALLOC stuff. Straight malloc calls were used
  elsewhere; it was undocumented outside of comments in pdcurses_curspriv.h; and
...
- Declarations for all supported, standard functions, per the X/Open
  ...
  before including pdcurses_curses.h (see xmas.c for an example). NOMACROS yields
...
- New installation method for XCurses: the header files are placed in a
  ...
  renamed. (But the renamed xpdcurses_curses.h and xpdcurses_panel.h are also installed,
  for backwards compatibility.) pdcurses_curspriv.h and pdcurses_term.h are now available,
  and existing curses-based code need no longer be edited to use
  XCurses' pdcurses_curses.h. And with no more need for explicit XCursesExit()
...
- Stub implementations of terminfo functions (including a pdcurses_term.h).
...
- The 16-bit chtype is a working option again (by removing #define
  CHTYPE_LONG from pdcurses_curses.h), except in XCurses. It's not recommended;
...
- redrawwin() macro in pdcurses_curses.h was incorrect - fixed by Alberto Ornaghi
...
- removed DESQview shadow memory buffer checking bug in pdcurses_curses.h in
...
support.  Converted all #ifdef to #if in all modules except pdcurses_curses.h and
pdcurses_curspriv.h. Always include ASSERT.H.  Added support for an external
...
standard.
  ...
  The definitions for OK and ERR in pdcurses_curses.h were exchanged. This was
...
function declarations and in pdcurses_curses.h. This was reported by Carlos
